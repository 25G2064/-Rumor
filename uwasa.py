...
# 噂A（正しい情報）
INITIAL_SPREADER_A = 0          # 最初は流れない
RUMOR_A_START = 50              # 50フレーム後に流れ始める

# 噂B（デマ）
INITIAL_SPREADER_B = 3          # デマは最初から流れる
...
def run_simulation():
    ...
    for frame in range(SIMULATION_TIME):
        ...
        # 正しい噂Aを信じたらデマBを非活性化
        for a in agents:
            if a.state_A == SPREADER or a.state_A == STIFLER:
                if a.state_B == SPREADER:
                    a.state_B = STIFLER
...
line_A_S, = ax2.plot([], [], color="red", label="A Spreader")
line_B_S, = ax2.plot([], [], color="yellow", label="B Spreader")
line_A_T, = ax2.plot([], [], color="darkred", label="A Stifler")
line_B_T, = ax2.plot([], [], color="goldenrod", label="B Stifler")
line_A_I, = ax2.plot([], [], color="pink", label="A Ignorant")
line_B_I, = ax2.plot([], [], color="lightyellow", label="B Ignorant")
