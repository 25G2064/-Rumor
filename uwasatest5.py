...
# デマの伝播（成功・失敗をカウント）
# ============================================
def spread_false(a1, a2, frame, counters):
...
# 真実の伝播（成功・失敗をカウント）
# ============================================
def spread_true(a1, a2, frame, counters):
...
def update_state(agent, frame, initial_true_ids):
    ...
    if agent.state == FALSE_SPREADER:
        if frame - agent.rumor_time > SimulationConfig.BASE_FORGET_TIME:
...
print("真実が伝わった回数:", counters["true_success"])
print("真実が伝わらなかった接触:", counters["true_fail"])
print("デマが伝わらなかった接触:", counters["false_fail"])
...
ax.set_xticks([d * frames_per_day for d in range(max_days + 1)])
ax.set_xticklabels([f"{d}日" for d in range(max_days + 1)])

ax.set_xlim(0, SimulationConfig.SIMULATION_TIME)
ax.set_ylim(0, SimulationConfig.AGENTS_NUMBER)
ax.set_title("Rumor Spread Over Time (Static)")
ax.set_xlabel("経過日数")
ax.set_ylabel("人数")
ax.grid(True)

ax.plot(count_I,  color="#999999", label="Ignorant")
ax.plot(count_FS, color="#0072B2", label="False Spreader")
ax.plot(count_FB, color="#99C2FF", label="False Believer")
ax.plot(count_TS, color="#E60026", label="True Spreader")
ax.plot(count_TB, color="#FF99A0", label="True Believer")
